...
public class EntityQueryPool {
	 ...
	 */
	public EntityQuery getQuery(Class<? extends Component>[] componentTypes, boolean disposable, String tag) {
		if (disposable) {
			return new EntityQuery(componentTypes, true, tag);
		}
		
		String key = generateQueryKey(componentTypes, tag);
		...
		EntityQuery entityQuery = new EntityQuery(componentTypes, false, tag);
	 ...
	 * @return String key
	 */
	private static String generateQueryKey(Class<? extends Component>[] componentTypes, String tag) {
		...
		StringBuilder stringBuilder = new StringBuilder(String.join(":", typeNames));
