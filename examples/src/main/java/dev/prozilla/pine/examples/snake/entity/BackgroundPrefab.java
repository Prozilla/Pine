...
public class BackgroundPrefab extends SpritePrefab {
	...
	protected void apply(Entity entity) {
		...
		int variant = (Math.abs((int)Math.floor(entity.transform.position.x / GameScene.CELL_SIZE)) % 2 + Math.abs((int)Math.floor(entity.transform.position.y / GameScene.CELL_SIZE)) % 2) % 2;
