package dev.prozilla.pine.tools.tools;
...
public class BuildTool {
	...
	public static void main(String[] args) throws Exception {
		...
		Path launch4jConfigPath = generateLaunch4jConfig(config, jrePath, buildDir, launch4jDir);
	...
	private static Path generateLaunch4jConfig(BuildConfig config, Path jrePath, Path buildDir, Path launch4jDir) throws IOException {
		System.out.println("Generating Launch4j configuration...");
		
		Path jar = buildDir.resolve("../libs/game-1.0-SNAPSHOT-all.jar").normalize();
		if (!Files.exists(jar)) {
